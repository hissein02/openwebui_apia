...
{#if !$mobile && !$showSidebar}
	<div
		class="bg-gray-900 pt-[7px] pb-2 px-2 flex flex-col justify-between text-white dark:text-white dark:hover:bg-gray-950/30 h-full z-10 transition-all border-e-[0.5px] border-gray-50 dark:border-gray-850/30"
		id="sidebar"
	>
		<button
		...
		>
			<div class="pb-1.5">
				...
				>
					<button
						class="flex rounded-xl hover:bg-gray-950 dark:hover:bg-gray-850 transition group {isWindows
							? 'cursor-pointer'
							: 'cursor-[e-resize]'}"
			...
			<div class="-mt-[0.5px]">
				<div class="">
					<Tooltip content={$i18n.t('New Chat')} placement="right">
						<a
							class=" cursor-pointer flex rounded-xl hover:bg-gray-950 dark:hover:bg-gray-850 transition group"
							href="/"
							draggable="false"
							on:click={async (e) => {
...
{#if $showSidebar}
	<div
		...
		id="sidebar"
		class="h-screen max-h-[100dvh] min-h-screen select-none bg-gray-900 dark:bg-gray-950/70 z-50 {$isApp
			? `ml-[4.5rem] md:ml-0 `
			: ' transition-all duration-300 '} shrink-0 text-white dark:text-gray-200 text-sm fixed top-0 left-0 overflow-x-hidden
        "
	...
	>
		...
		>
			...
			>
				...
				>
					<button
						class="flex rounded-xl size-8.5 justify-center items-center hover:bg-black/20 dark:hover:bg-gray-850/50 transition {isWindows
							? 'cursor-pointer'
							: 'cursor-[w-resize]'}"
						on:click={() => {
							showSidebar.set(!$showSidebar);
				...
				<div
					class="{scrollTop > 0
						? 'visible'
						: 'invisible'} sidebar-bg-gradient-to-b bg-linear-to-b from-gray-900 dark:from-gray-950 to-transparent from-50% pointer-events-none absolute inset-0 -z-10 -mb-6"
				></div>
			</div>
			...
			>
				<div class="pb-1.5">
					<div class="px-[0.4375rem] flex justify-center text-gray-100 dark:text-gray-200">
						<a
							id="sidebar-new-chat-button"
							class="group grow flex items-center space-x-3 rounded-2xl px-2.5 py-2 hover:bg-black/20 dark:hover:bg-gray-900 transition outline-none"
							href="/"
							draggable="false"
							on:click={newChatHandler}
							aria-label={$i18n.t('New Chat')}
					...
					{#if ($config?.features?.enable_notes ?? false) && ($user?.role === 'admin' || ($user?.permissions?.features?.notes ?? true))}
						<div class="px-[0.4375rem] flex justify-center text-gray-100 dark:text-gray-200">
							<a
								...
								class="grow flex items-center space-x-3 rounded-2xl px-2.5 py-2 hover:bg-black/20 dark:hover:bg-gray-900 transition"
								href="/notes"
					...
					{#if $pinnedChats.length > 0}
						<div class="mb-1">
							<div class="flex flex-col space-y-1 rounded-xl">
								...
								>
									<div
										class="ml-3 pl-1 mt-[1px] flex flex-col overflow-y-auto scrollbar-hidden border-s border-gray-100 dark:border-gray-900 text-gray-100 dark:text-gray-200"
									>
							...
							{#if $chats}
								{#each $chats as chat, idx (`chat-${chat?.id ?? idx}`)}
									{#if idx === 0 || (idx > 0 && chat.time_range !== $chats[idx - 1].time_range)}
										<div
											class="w-full pl-2.5 text-xs text-gray-200 dark:text-gray-500 font-medium {idx ===
											0
												? ''
												: 'pt-5'} pb-1.5"
										>
			...
			<div class="px-1.5 pt-1.5 pb-2 sticky bottom-0 z-10 -mt-3 sidebar">
				<div
					class=" sidebar-bg-gradient-to-t bg-linear-to-t from-gray-900 dark:from-gray-950 to-transparent from-50% pointer-events-none absolute inset-0 -z-10 -mt-6"
				></div>
				<div class="flex flex-col font-primary">
					{#if $user !== undefined && $user !== null}
						...
						>
							<div
								class=" flex items-center rounded-2xl py-2 px-1.5 w-full hover:bg-black/20 dark:hover:bg-gray-900/50 transition"
							>
								...
								<div class=" self-center font-medium text-white dark:text-gray-200">
									{$user?.name}
								</div>
							</div>
						</UserMenu>
	...
	{#if !$mobile}
		<div
			class="relative flex items-center justify-center group border-l border-white/10 dark:border-gray-850/30 hover:border-white/30 dark:hover:border-gray-800 transition z-20"
			id="sidebar-resizer"
